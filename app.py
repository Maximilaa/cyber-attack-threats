...
with col1:
    ...
    flow_duration = st.number_input("Flow Duration (ms) [0 - 1,000,000]", min_value=0.0, max_value=1000000.0, value=0.0)
    packet_size = st.number_input("Packet Size (bytes) [32 - 1,514]", min_value=32.0, max_value=1514.0, value=64.0)
    flow_bytes = st.number_input("Flow Bytes/s [0 - 950,000]", min_value=0.0, max_value=950000.0, value=1024.0)
    flow_packets = st.number_input("Flow Packets/s [0 - 2,000]", min_value=0.0, max_value=2000.0, value=50.0)
    cpu_util = st.number_input("CPU Utilization (%) [0 - 100]", min_value=0.0, max_value=100.0, value=45.0)
    mem_util = st.number_input("Memory Utilization (%) [0 - 100]", min_value=0.0, max_value=100.0, value=65.0)
    anomaly_index = st.number_input("Anomaly Severity Index [0 - 10]", min_value=0.0, max_value=10.0, value=3.4)
    normalized_flow = st.number_input("Normalized Packet Flow [0 - 1.0]", min_value=0.0, max_value=1.0, value=0.85)
